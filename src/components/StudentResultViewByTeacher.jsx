...
function StudentResultViewByTeacher() {
  ...
  const cachedResultInStore = useSelector(state => state.teacher.particularStudentResult)
  const shouldFetch = !(cachedResultInStore.result && cachedResultInStore.id == id);
  ...
  useEffect(() => {
    if(shouldFetch){
      ...
      dispatch(settingParticularStudentResult({id,result:data?.data[0]?.Result,name:data?.data[0]?.name,email:data?.data[0]?.email}))
    }else{
  ...
  return (
    <div className="max-w-2xl mx-auto ">
      <BackButton />
      {loading && "loading..."}
      {result && <div className="main p-2">
        <h2 className="mt-2">Name: {name}</h2>
        <h2 className="mt-2">Email: {email}</h2>
        <div className="res mt-5">
            {
                result?.length >= 1 ? <div className="wrappers ">
                    {
                        result.map((res,i)=> {
                            return <div key={i} className="single bg-white p-5 rounded-2xl mb-5">
                ...
                </div> : <p className="text-gray-400">no result for this student.</p>
            }
